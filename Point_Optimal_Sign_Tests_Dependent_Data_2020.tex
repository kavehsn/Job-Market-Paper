...
Brevan Howard Centre for Financial Analysis, Imperial College Business School, London, SW7 2AZ, UK
\href{emailto: k.salehzadeh-nobari@imperial.ac.uk}{k.salehzadeh-nobari@imperial.ac.uk}}\\
%EndAName
Imperial College London \and Abderrahim Taamouti\thanks{%
University of Liverpool Management School, University of Liverpool, Liverpool, L69 7ZH, UK
\href{emailto: abderrahim.taamouti@liverpool.ac.uk}{abderrahim.taamouti@liverpool.ac.uk}}\\
%EndAName
...
{\hskip 1.5em} Numerous studies investigate the predictability of financial and economic variables using the past values of one or more predictors, as well as the horizon at which evidence in favor of predictability is prevalent. Most commonly encountered examples concern the predictability of stock returns using the lag of certain fundamental variables, such as the dividend-price and earnings-price ratios or the interest rates [see \citet{campbell1988dividend}, \citet{fama1988dividend,fama1988permanent}, \citet{campbell2006efficient}, \citet{campbell2008predicting}, and \citet{golez2018four} among others]. Predictability in this context is generally assessed using the OLS regression of returns against said predictors and tested with conventional \textit{t}-type tests. For instance, by utilizing this approach, \citet{fama1988dividend,fama1988permanent} and \citet{welch2008comprehensive} conclude that predictability in stock returns is not of economic significance at short horizons, yet the predictability power of the predictors increase at longer horizons. 
...
regression of returns [see \citet{phillips2015halbert} for a review]. In such situations, unless the predictor is strictly exogenous, we know that the OLS estimator of the
coefficients, although consistent, will suffer from significant bias [see \citet{stambaugh1999predictive}, \citet{magdalinos2009limit}]. As a result, the \textit{t}-statistic will have a nonstandard distribution in finite samples which leads to invalid inference [see \citet{mankiw1986we}, \citet{banerjee1993co} and \citet{stambaugh1999predictive} among others]. Moreover, inference based on consistent heteroskedasticity and autocorrelation corrected (HAC) approaches are shown to have poor finite sample performance under different forms of heteroskedasity and nonlinear dependencies [see \citet{dufour2010exact}], which is of relevance in this setting, since some studies further suggest that stock returns' volatility are stochastic and highly persistent [see \citet{jacquier2004bayesian} and \citet{hansen2014realized}]. In this paper, we address the endogeneity issue inherent within a predictive regression framework, by deriving point-optimal sign-based tests (POS-based tests hereafter) in the context of linear and nonlinear models that are distribution-free, robust against heteroskedasticity of unknown form as well as serial (nonlinear) dependence, provided that errors have zero median conditional on their past and the past of the explanatory variables.% This assumption allows the signs to be i.i.d under the null hypothesis of orthogonality according to a known distribution, despite the fact that the variables to which the indicator functions are applied are dependent [see \citet{coudin2009finite}].	

\citet{nelson1993predictable} show that in a predictive regression setting, sample slope coefficients and the asymptotic standard errors of \citet{newey1986simple} are biased towards significant $t$-statistics, which increase with longer return horizons. Thus, they reduce the small-sample bias using bootstrap simulations, whereas \citet{stambaugh1999predictive} further demonstrates that in the case of stationary regressors said bias can be corrected using a first order asymptotic bias correction procedure. \citet{amihud2004predictive} and \citet{amihud2008multiple} adopt a similar process to reduce finite sample bias in regression coefficients for short return horizons, and more recently \citet{kan2021finite} adjust for the bias in the slope coefficient and the standard errors in the context of long-horizon returns. However, studies by \citet{phillips2013predictive} and \citet{phillips2014confidence} show this approach to be infeasible in the presence of predictors that exhibit local-to-unity, unit-root or explosive persistency. Therefore, many inference procedures in this context address the issue of size distortions by considering local-to-unity asymptotics as proposed by \citet{chan1987asymptotic} and \citet{phillips1987towards}, where the key predictor variable is assumed to be integrated
[\citet{lewellen2004predicting}], or can be modeled as having a local-to-unit root [\citet{elliott1994inference}, \citet{torous2004predicting}, and \citet{campbell2006efficient}, among others]. Notable studies under the local-to-unity dynamics employ an array of procedures, such as Bonferroni corrections [e.g. \citet{cavanagh1995inference} and \citet{campbell2006efficient}], a conditional likelihood based approach [e.g. \citet{jansson2006optimal}], as well as the nearly optimal tests proposed by \citet{elliott2015nearly}. In more recent works, \citet{kostakis2015robust} and \citet{phillips2016robust} expand on the predictability literature by utilizing an extension of the instrumental variable (IV hereafter) procedure suggested by \citet{phillips2009econometric}, to generalize inference to multivariate regressors with integrated and mildly explosive persistency. As far as IV procedures are concerned, \citet{ibragimov2020new} adopt a nonlinear instrumental variable estimation approach (where the instrument is a sign transformation of the predictor) that deals with the issues of persistent endogeneity and heavy-tailedness. Additionally, they propose nonparametric volatility correction to circumvent the problems caused by heterogeneous and persistent stock returns' volatility. Finally, a number of exact and nonparametric inference procedures within this context include the signed and sign-ranked statistics of \citet{campbell1991over} and \citet{dufour1995exact,campbell1997exact}, which are model-free and impose mild assumptions on the distribution of the error terms. These inference methods are useful in the presence of a single regressor and a nuisance parameter. Recent developments in distribution-free and exact tests include those proposed by \citet{gungor2021exact}, which consider several prediction horizons,  as well as test for predictability in quantiles. 
...
{\hskip 1.5em} In this Section, we adapt the framework considered by \citet{coudin2009finite} to a predictive regression setup, where the dependent variable is now regressed against a lagged predictor. Consider a stochastic process $Z=\{Z_t=(y_t,\bm{x}_{t-1}'):\Omega\rightarrow\R^{(k+1)}, t=1,2,\cdots\}$ defined on a probability space $(\Omega,\mathcal{F},P)$. Suppose that $y_{t}$ can linearly be explained by the vector variable $\bm{x}_{t-1}$%
\begin{equation}
y_{t}=\bm{\beta}'\bm{x}_{t-1}+\varepsilon_{t},\quad t=1,\cdots,T,  \label{model}
\end{equation}%
where $y_t$ is a dependent variable and $\bm{x}_{t-1}$ is an $(k+1)\times 1$ vector of stochastic explanatory 
...
where $F_{t}(.\mid X)$ is an unknown conditional distribution function and $X=[\bm{x}_0',\cdots,\bm{x}_{T-1}']'$ is an $T\times (k+1)$ information matrix. 
 ...
 Let $\{Z_t,\mathcal{F}_t\}_{t=1,2,\cdots}$ be an adapted stochastic sequence, such that $\mathcal{F}_t$ is a $\sigma$-field in $\Omega$, $\mathcal{F}_s\subseteq \mathcal{F}_t$ for $s< t$, $\sigma( Z_1,\cdots,Z_t)\subset \mathcal{F}_t$, where $\sigma(Z_1,\cdots,Z_t)$ is the $\sigma$-field generated by $Z_1,\cdots,Z_t$. In the context of general forms of serial (nonlinear) dependence, an assumption commonly imposed on the error terms $\{\varepsilon_t,t=1,2,\cdots\}$ is that the error process is a martingale difference sequence (MDS hereafter) with respect to $\mathcal{F}_t=\sigma(Z_1,\cdots,Z_t)$ for $t=1,2,\cdots$, - i.e. $\E\{\varepsilon_t\mid \mathcal{F}_{t-1}\}=0$,\quad $\forall t\geq1$. We follow \citet{coudin2009finite} by departing from this assumption and considering the median as an alternative measure of central tendency. This implies imposing a median-based analogue of the MDS on the error process - namely, we extend the \textit{quantilegale} assumption proposed by \citet{linton2007quantilogram} and suppose that $\varepsilon_t$ is a strict conditional mediangale as defined as follows
\begin{definition}
...
\end{definition}
\begin{sloppypar}
Before proceeding further, it is important to emphasize that \citet{linton2007quantilogram} define $\varepsilon_t$ as a mediangale if and only if $\E\left[\psi_{\frac{1}{2}}(\varepsilon_t)\mid\mathcal{F}_{t-1}\right]=0,\quad\forall t$, with $\mathcal{F}_{t-1}=\sigma(\varepsilon_{t-1},\varepsilon_{t-2},\cdots)$ and $\psi_{\frac{1}{2}}(x)=\mathbbm{1}_{[0,\infty)}$.  However, note that the strict conditional mediangale assumption (\ref{eq: mediane}), is conditioned on the entire process $X$. \citet{coudin2009finite} argue that as long as the $x_t$'s are strictly exogenous, the conditional mediangale assumption is analogous to MDS on the signs with respect to $\mathcal{F}_t=\sigma(Z_1,\cdots,Z_{t})$. However, as the errors of the predictive regressions of the form (\ref{model}) are contemporaneously correlated with the disturbances of the regressors, say $u_t$, there if feedback from $u_t$ to $x_t$ through $\varepsilon_t$ and the process $x_t$ is weakly exogenous, which subsequently suggests that (\ref{eq: mediane}) can be substituted with
\begin{equation}
P[\varepsilon_{t}> 0\mid \bm{{\varepsilon}}_{t-1},\mathcal{X}_{t-1}]=P[\varepsilon_{t}<0\mid \bm{\varepsilon}_{t-1},\mathcal{X}_{t-1}]=\frac{1}{2},
\end{equation}
where $\mathcal{X}_{t-1}=\{x_0,\cdots,x_{t-1}\}$.
Additionally, assumption (\ref{eq: mediane}) entails that $\varepsilon _{t}\mid X$ has no mass at zero for all $t$, which is only true if $\varepsilon_{t}\mid X$\ is continuous. Following Definition 2.1 of \citet{coudin2009finite}, the strict conditional mediangale assumption can be relaxed, by substituting $P[\varepsilon_{t}> 0\mid \bm{{\varepsilon}}_{t-1},X]=P[\varepsilon_{t}<0\mid \bm{\varepsilon}_{t-1},X]$ in equation (\ref{eq: mediane}), which in turn allows $\varepsilon_t$ to have discrete distribution, with a non-zero probability mass at zero. In this case, \citet{coudin2009finite} show that the sign transformation $\mathcal{S}(x, U)=s(x)+[1-s(x)^2]s(U-0.5)$ for $U\sim \mathcal{U}(0,1)$, and as long as $U$ and $\varepsilon_t$ are independent, conforms to 
\begin{equation}
P[\mathcal{S}(\varepsilon_t, U_t)=1]=P[\mathcal{S}(\varepsilon_t, U_t)=0]=\frac{1}{2},
\end{equation}
irrespective of the distribution of $\varepsilon_t$. However,  for the simplicity of exposition, we only consider continuous distributions.
 
 Equation (\ref{model}) in conjunction with assumption (\ref{eq: mediane}) allows the error terms to possess asymmetric, heteroskedastic and serially (nonlinear) dependent distributions, so long as the conditional medians are zero. Assumption \ref{eq: mediane} allows for many dependent schemes, such as those of the form $\varepsilon_1=\sigma_1(x_0,\cdots,x_{t-2})\epsilon_1$, $\varepsilon_t=\sigma_t(x_0,\cdots,x_{t-2},\varepsilon_1,\cdots,\varepsilon_{t-1})\epsilon_t$, $t=2,\cdots,T$, where $\epsilon_1,\cdots,\epsilon_T$ are independent with a zero median. In time-series context this includes models such as ARCH, GARCH or stochastic volatility with non-Gaussian errors. Furthermore, in the mediangale framework the disturbances need not be second order stationary.

Finally, we reiterate \citet{coudin2009finite} by noting that modifying assumption (\ref{eq: mediane}) to $P[\varepsilon_t <0 \mid \mathcal{F}_{t-1} ]$ allows us to consider other quantiles. 
\end{sloppypar} 
...
{\hskip 1.5em}In this Section, we derive POS-based tests in the context of linear and nonlinear predictive regressions. First, we divert our attention to the problem of testing the null hypothesis of unpredictability in a linear model, which is later generalized to testing unpredictability in a nonlinear model. Although the former problem is a special case of the latter, for simplicity of exposition the linear predictive  regression model is considered first.
...
\begin{equation}
U(T)=(s(y_{1}),\cdots,s(y_{T}))',
\end{equation}%
where, for $1\leq t\leq T,$%
\begin{equation}
s(y_{t})=
\begin{cases}
1,\quad \text{if}\quad y_{t}\geq 0 \\ 
0,\quad \text{if}\quad y_{t}<0%
\end{cases}%
\end{equation}
...
depends on the weights $a_{t}(\bm{\beta}_{1}),$ which in turn depend on the
calculation of the conditional probabilities $P\left[ y_{t}\geq 0\mid \text{\b{S%
}}_{t-1},X\right] $ and $P\left[ y_{t}<0\mid \text{\b{S}}%
_{t-1},X\right] .$ The latter two terms involve the
distribution of the joint process of signs $\big(s(y_{1}),\cdots,s(y_{T})\big)'$ conditional on $X$, which is unknown. An alternative way to compute the
terms $P\left[ y_{t}\geq 0\mid \text{\b{S}}_{t-1},X\right] $ and $%
P\left[ y_{t}<0\mid \text{\b{S}}_{t-1},X\right] $ is to use
simulations, however, this will be computationally burdensome as it requires the
simulation of the joint distribution of the process of signs $\big(%
s(y_{1}),\cdots,s(y_{T})\big)'$ conditional on $X$, which depends on the sample size $T$.
Hence, to propose a feasible test statistic, say $\widetilde{SL}_{T}(\bm{\beta}_{1})$, we impose the following assumption.
\end{sloppypar}
...
such that%
\begin{equation}
\tilde{a}_{1}(\bm{\beta}_{1})=\ln \left\{ \frac{\pi_1^c(\bm{\beta}_1) }{\pi_1(\bm{\beta}_1) }\right\} ,\quad\tilde{b}_{1}(\bm{\beta}_{1})=0,
\end{equation}%
and for $t=2,\cdots,T,$%
\begin{equation}
\tilde{a}_{t}(\bm{\beta}_{1})=\ln \left\{\left(1-\frac{\pi_{t,t-1}(\bm{\beta}_1) }{\pi_{t-1}(\bm{\beta}_1)}\right)\bigg/\frac{\pi_{t,t-1}(\bm{\beta}_1)}{\pi_{t-1}(\bm{\beta}_1)}\right\}, \quad \tilde{b}_{t}(\bm{\beta}_{1})=\ln \left\{ \frac{1-\left( \frac{\pi_t(\bm{\beta}_1) }{\pi_{t-1}^c(\bm{\beta}_1) }-\frac{\pi_{t,t-1}(\bm{\beta}_1)}{\pi_{t-1}^c(\bm{\beta}_1)}\right) }{\frac{\pi_t(\bm{\beta}_1) }{\pi_{t-1}^c(\bm{\beta}_1) }-\frac{\pi_{t,t-1}(\bm{\beta}_1) }{\pi_{t-1}^c(\bm{\beta}_1) }}\right\} -\tilde{a}_{t}(\bm{\beta}_{1}), 
\end{equation}%
where for $t=2,\cdots,T$
\begin{align}
\pi_{t}(\bm{\beta}_1)&=P\left[ \varepsilon_{t}<-\bm{\beta}
_{1}'\bm{x}_{t-1}\mid X\right]\label{eq: pit} \\
\pi_{t,t-1}(\bm{\beta}_1)&=P\left[
\varepsilon_{t}<-\bm{\beta}_1'\bm{x}_{t-1},\text{ }\varepsilon_{t-1}<-\bm{\beta}_{1}'
\bm{x}_{t-2}\mid X\right] \label{eq: pitt}\\
\pi_{t}^c(\bm{\beta}_1)&=1-\pi_{t}(\bm{\beta}_1),\label{eq: pitc}
\end{align}
noting that $\pi_{1,0}=\pi_{1}=P\left[ \varepsilon_{1}<-\bm{\beta}
_{1}'\bm{x}_{0}\mid X\right]$. Further, suppose the constant $\tilde{c}_{1}(\bm{\beta}_{1})$ satisfies 
\begin{equation}
P\left[ \sum\limits_{t=1}^{T}\tilde{a}_{t}(\bm{\beta}_{1})%
s(y_{t})+\sum\limits_{t=1}^{T}\tilde{b}_{t}(\bm{\beta}_{1})%
s(y_{t})s(y_{t-1})>\tilde{c}_{1}(\bm{\beta}_{1})\mid H_0\right] =\alpha 
\end{equation}
...
\varepsilon_{t}<\cdot \mid X\right]$ using a multivariate Student's $t$ distribution $\bm{t}_\nu(\bm{0},I)$, by imposing a zero location parameter vector and the identity correlation matrix $I$.
 Then the probabilities (\ref{eq: pit}), (\ref{eq: pitt}) and (\ref{eq: pitc}) are evaluated as follows:
\begin{align}
\pi_{t}(\bm{\beta}_1)&=\tau_{\nu}(-\bm{\beta}
_{1}'\bm{x}_{t-1}) \\
\pi_{t,t-1}(\bm{\beta}_1)&=\bm{\mathcal{T}}_{\nu}(-\bm{\beta}
_{1}'\bm{x}_{t-1},-\bm{\beta}
_{1}'\bm{x}_{t-2})\\
\pi_{t}^c(\bm{\beta}_1)&=1-\pi_{t}(\bm{\beta}_1),
\end{align}
...
{\hskip 1.5em}We may obtain exact critical values and confidence intervals in finite samples whose distribution is intractable by simulations using Monte Carlo test techniques [see \citet{dwass1957modified}, \citet{barnard1963discussion}, \citet{dufour2006monte}, \citet{coudin2009finite}]. Under the null hypothesis of unpredictability, the distribution of the test statistic, $\widetilde{SL}_{T}(\bm{\beta} _{1})$, does not involve any nuisance parameters and is a pivotal function. As such, performing Monte Carlo tests is rather straightforward\footnote{\citet{dufour2006monte} consider extensions involving nuisance parameters using maximized Monte Carlo tests (MMC).}. Hence, the distribution of $\widetilde{SL}_{T}(\bm{\beta} _{1})$, can be simulated under the null
...
%where $u_{1},\cdots,u_{n}$ are i.i.d. and distributed according to a standard
...
Since the test statistic $\widetilde{SL}_{T}(\bm{\beta}_{1})$ is a continuous random variable, its quantiles $F^{-1}(\alpha)$ are easy to compute, where for any probability distribution $F(x)$ the quantile function is defined as follows:
\begin{equation}
F^{-1}(\alpha)=
\begin{cases}
\inf\{x: F(x)\geq \alpha)\}\quad &\text{if }0<\alpha<1\\
\inf\{x: F(x)>0\}\quad &\text{if }\alpha=0\\
\sup\{x: F(x)<1\}\quad &\text{if }\alpha=1.
\end{cases}
\end{equation}
 To simulate the distribution of 
$\widetilde{SL}_{T}(\bm{\beta}_{1}),$ the following algorithm is implemented:
\begin{algorithm}
\caption{Algorithm for utilizing the POS-based tests (continuous $\varepsilon_t$) }
\begin{algorithmic}[1]
...
As discussed in Section \ref{Framework}, suppose the errors $\varepsilon_t$ possess a discrete distribution, and the test-statistic is subsequently modified to reflect this. Then $\widetilde{SL}_{T}(\bm{\beta} _{1})$ is no longer continuous, such that for the replications $\widetilde{SL}_{T}^{(1)}(\bm{\beta} _{1}),\cdots,\widetilde{SL}_{T}^{(B)}(\bm{\beta} _{1})$
\begin{equation}
P\left[\widetilde{SL}_{T}^{(l)}(\bm{\beta} _{1})=\widetilde{SL}_{T}^{(m)}(\bm{\beta} _{1})\right]\neq 0,\quad \text{for} \quad l\neq m\quad \text{and}\quad i,j=1,\cdots,B,
\end{equation}
and Monte Carlo tests with randomized tie-breaking approach suggested by \citet{dufour2006monte} and \citet{coudin2009finite} may be used to perform hypothesis tests. The latter entails \textquotedblleft breaking\textquotedblright{ }ties according to a uniform distribution. In other words, for each variable $\widetilde{SL}_{T}^{(j)}(\bm{\beta} _{1}), j=1,\cdots, B$, we introduce a random variable $W^{(j)},j=1,\cdots, B$, where $W^{(j)}\sim \mathcal{U}(0,1)$, such that $W(B)=(W^{(1)},\cdots,W^{(B)})'$ is independent of $S(B)=(\widetilde{SL}_{T}^{(1)}(\bm{\beta} _{1}),\cdots,\widetilde{SL}_{T}^{(B)}(\bm{\beta} _{1}))'$. Then we consider the pairs $(\widetilde{SL}_{T}^{(j)}(\bm{\beta}_{1}),W^{(j)}),j=1,\cdots,B$, ordered lexicographically - i.e., 
\begin{align}
\begin{split}
&\left(\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1}),W^{(n)}\right)\geq \left(\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1}),W^{(m)}\right) \Leftrightarrow\\
&\left\{\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})>\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})\quad\text{or}\quad \left(\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})=\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})\quad\text{and}\quad W^{(n)}\geq W^{(m)}\right)\right\}
\end{split}
\end{align}
where using the indicator and Kronecker delta functions, 
\begin{align}\label{eq: tiebreaking}
\begin{split}
&\mathbbm{1}\left[\left(\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1}),W^{(n)}\right)\geq \left(\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1}),W^{(m)}\right)\right] =\\
&\mathbbm{1}\left[\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})>\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})\right]+\delta\left (\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})-\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})\right)\mathbbm{1}\left[W^{(n)}\geq W^{(m)}\right].
\end{split}
\end{align}
and thus according to (\ref{eq: tiebreaking}), $\widetilde{SL}_{T}^{(1)}(\bm{\beta} _{1}),\cdots,\widetilde{SL}_{T}^{(B)}(\bm{\beta} _{1})$ is ordered as the pairs $(\widetilde{SL}_{T}^{(j)}(\bm{\beta}_{1}),W^{(j)}),j=1,\cdots,B$. The algorithm for the latter is as follows: 
\begin{algorithm}
\caption{Algorithm for utilizing the POS-based tests (discrete $\varepsilon_t$)}
\begin{algorithmic}[1]
\State Compute $\widetilde{SL}_{T}^{0}(\bm{\beta}_{1})\leftarrow\widetilde{SL}_{T}(\bm{\beta}_{1})$ based on the
observed data;
\State Choose $B$ such that $\alpha (B+1) \in \N$;
\For{$j$ in $1:B$}
\State Generate $\{\mathcal{S}_t^{(j)}\}_{t=1}^T$, s.t. $\mathcal{S}_{t}^{(j)}\sim Bin(n = 1, p = 0.5)$;
\State Compute $\widetilde{SL}_{T}^{(j)}(\bm{\beta}_{1})$ using
the generated sample;
\State Generate $W^{(j)}\sim\mathcal{U}(0,1)$ to produce the pairs $(\widetilde{SL}_{T}^{(j)}(\bm{\beta}_{1}),W^{(j)})$;
\EndFor
\State Order the pairs: $(\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1}),W^{(n)})\geq (\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1}),W^{(m)}) \Leftrightarrow\newline\{\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})>\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})$ or $(\widetilde{SL}_{T}^{(n)}(\bm{\beta}_{1})=\widetilde{SL}_{T}^{(m)}(\bm{\beta}_{1})$  and $W^{(n)}\geq W^{(m)})\}$.
\State Compute $\tilde{c}_{1}(\bm{\beta}_{1})\leftarrow (1-\alpha )\%$ quantile of $\{\widetilde{SL}_{T}^{(j)}(\bm{\beta}_{1})\}_{j=1}^{B}$;
\If{$\widetilde{SL}_{T}^{0}(\bm{\beta}
_{1})\geq c(\bm{\beta}_{1})$}
\State Reject $H_0$
...
{\hskip 1.5em}Following \citet{dufour2010exact}, we may extend the earlier results by considering a nonlinear predictive regression model of the form%
\begin{equation}
y_{t}=f(\bm{x}_{t-1},\bm{\beta} )+\varepsilon_{t},\quad t=1,\cdots,T,
\label{nonlinearC1}
\end{equation}%
where $\bm{x}_{t-1}$ is an observable $(k+1)\times 1$ vector of stochastic
...
\end{equation}%
where as before $F_{t}(.\mid X)$ is a distribution function and $X=[\bm{x}_0',\cdots,\bm{x}_{T-1}']'$ is an $T\times (k+1)$ matrix. Once again, we suppose that the error terms process $\{\varepsilon_t,t=1,2,\cdots\}$ is a strict conditional mediangale, such that
...
_{t}(\bm{\beta}_{0}\mid\bm{\beta}_{1})s(\tilde{y}_{t})+\sum\limits_{t=1}^{T}\tilde{b}_{t}(\bm{\beta_{0}\mid\beta}_{1})
s(\tilde{y}_{t})s(\tilde{y}_{t-1}),
\end{equation}%
such that%
\begin{equation}
\tilde{a}_{1}(\bm{\beta}_{0}\mid\bm{\beta}_{1})=\ln \left\{ \frac{\widetilde{\pi}^c_{t}(\bm{\beta}_1) }{\widetilde{\pi}_{t}(\bm{\beta}_1) }\right\} ,\quad%
\tilde{b}_{1}(\bm{\beta}_{0}\mid\bm{\beta}_{1})=0,
\end{equation}%
and for $t=2,\cdots,T,$%
\begin{align}
\tilde{a}_{t}(\bm{\beta}_{0}\mid\bm{\beta}_{1})&=\ln \left\{ \left(1-\frac{\widetilde{\pi}_{t,t-1}(\bm{\beta}_1) }{\widetilde{\pi}_{t-1}(\bm{\beta}_1) }\right)\bigg/\frac{\widetilde{\pi}_{t,t-1}(\bm{\beta}_1) }{\widetilde{\pi}_{t-1}(\bm{\beta}_1) }\right\},
\\
\tilde{b}_{t}(\bm{\beta}_{0}\mid\bm{\beta}_{1})&=\ln \left\{ \frac{1-\left( \frac{\widetilde{\pi}_{t}(\bm{\beta}_1)}{\widetilde{\pi}^c_{t-1}(\bm{\beta}_1) }-\frac{\widetilde{\pi}_{t,t-1}(\bm{\beta}_1)}{\widetilde{\pi}^c_{t-1}(\bm{\beta}_1) }\right) }{\frac{\widetilde{\pi}_{t}(\bm{\beta}_1) }{\widetilde{\pi}^c_{t-1}(\bm{\beta}_1) }-\frac{\widetilde{\pi}_{t,t-1}(\bm{\beta}_1) }{\widetilde{\pi}^c_{t-1}(\bm{\beta}_1)}}\right\} -\tilde{a}_{t}(\bm{\beta}_{0}\mid\bm{\beta}_{1})
\end{align}%
where for $t=2,\cdots,T$
\begin{align}
\widetilde{\pi}_{t}(\bm{\beta}_1)&=P\left[ \varepsilon_{t}<f(\bm{x}_{t-1},\bm{\beta}_{0})-f(\bm{x}_{t-1},\bm{\beta}_{1})\mid X\right]\label{eq: pitN} \\
\widetilde{\pi}_{t,t-1}(\bm{\beta}_1)&=P%
\left[ \varepsilon_{t}<f(\bm{x}_{t-1},\bm{\beta}_{0})-f(\bm{x}_{t-1},\bm{\beta}_{1}),\text{ }%
\varepsilon_{t-1}<f(\bm{x}_{t-2},\bm{\beta}_{0})-f(\bm{x}_{t-2},\bm{\beta}_{1})\mid X\right]\label{eq: pittN}\\
\widetilde{\pi}_{t}^c(\bm{\beta}_1)&=1-\widetilde{\pi}_{t}(\bm{\beta}_1),\label{eq: pitcN}
\end{align}
noting that $\widetilde{\pi}_{1,0}=\widetilde{\pi}_{1}=P\left[ \varepsilon_{t}<f(\bm{x}_{t-1},\bm{\beta}_{0})-f(\bm{x}_{t-1},\bm{\beta}_{1})\mid X\right]$. Further suppose the constant $\tilde{c}_{1}(\bm{\beta}_{0},\bm{\beta}_{1})$ satisfies 
\begin{equation}
P\left[ \widehat{SN}_{T}(\bm{\beta}_{0}\mid\bm{\beta}_{1})>\tilde{c}_{1}(\bm{\beta}
_{0},\bm{\beta}_{1})\mid H(\bm{\beta}_{0})\right] =\alpha 
\end{equation}
with $0<\alpha <1$. Then the test that rejects $H(\bm{\beta}_{0})$ when 
\begin{equation}
...
A special case entails considering a linear function $f(\bm{x}_{t-1},\bm{\beta})=\bm{\beta}'\bm{x}_{t-1}$, where as before we may suppose that $\varepsilon_{t}$ for $t=1,\cdots,T$  are distributed according to Student's $t$ distribution $t(\nu)$, with $\nu$ degrees of freedom, and where the bivariate probabilities can be calculated by imposing a bivariate Student's $t$ distribution $\bm{t}_\nu(\bm{0},I)$, with a zero location parameter and identity matrix to capture the serial (nonlinear) dependence. Then the statistic for testing the null hypothesis $H(\bm{\beta}_0)$ against the alternative $H(\bm{\beta}_1)$ is expressed using the weights
\begin{align}
\widetilde{\pi}_{t}(\bm{\beta}_1)&=\tau_{\nu}\left(\left(\bm{\beta}_1-\bm{\beta}_0\right)'\bm{x}_0\right) \\
\widetilde{\pi}_{t,t-1}(\bm{\beta}_1)&=\bm{\mathcal{T}}_{\nu}((\bm{\beta}_0-\bm{\beta}_1)'\bm{x}_{t-1},(\bm{\beta}_0-\bm{\beta}_1)'\bm{x}_{t-2} )\\
\widetilde{\pi}_{t}^c(\bm{\beta}_1)&=1-\widetilde{\pi}_{t}(\bm{\beta}_1),
\end{align}
where $\tau_{\nu}(.)$ is the Student's $t$ distribution function with $\nu$ degrees of freedom, $\bm{\mathcal{T}}_{\nu}(.,.)$ is the bivariate $t$ distribution with $\nu$ degrees of freedom, and with location and shape parameters $0$ and $I$ respectively.

 {\hskip 1.5em}As in Section \ref{sec: Testing zero coefficient hypothesis in linear model}%
...
{\hskip 1.5em}Point-optimal tests depend on the alternative $\bm{\beta}=\bm{\beta}_1$, which in practice is unknown. Formally, the test statistic $\widehat{SN}_{T}(\bm{\beta}_{0}\mid\bm{\beta}_{1})$ for testing the full-coefficient hypothesis (\ref{h01}) is a function
of $\bm{\beta}_{1}$
\begin{equation}
\widehat{SN}_{T}(\bm{\beta}_{0}\mid\bm{\beta}_{1})=\sum\limits_{t=1}^{T}\tilde{a}%
_{t}(\bm{\beta} _{0}\mid\bm{\beta}_{1})s(y_{t}-f(\bm{x}_{t-1},\bm{\beta}
_{0}))+\sum\limits_{t=1}^{T}\tilde{b}_{t}(\bm{\beta}_0\mid\bm{\beta}_{1})%
...
Our objective is to choose the value of $\bm{\beta}_1$ at which the power function of the POS-based test statistic is maximized and is close to that of the power envelope. This can be accomplished in a number of ways.
\citet{dufour2010exact} suggest an adaptive approach based on the split-sample technique [see \citet{dufour2001finite}] to estimate the optimal alternative and to calculate the test statistic, to make size control easier and maximize power. For a review of adaptive approach for parametric tests with nonstandard distributions see \citet{dufour2003point} and \citet{dufour2008finite}.
...
\begin{equation}
C_{\bm{\beta} }(\alpha )=\left\{ \bm{\beta} _{0}:\widehat{SN}_{T}(\bm{\beta} _{0}\vert \bm{\beta}
_{1})<\tilde{c}_1(\bm{\beta} _{0},\bm{\beta}_{1}) \Big\vert P[\widehat{SN}_{T}(\left. \bm{\beta}_{0}\right\vert \bm{\beta}
...
To compare the performance of certain parametric and nonparametric tests to that of the 10\% SS-POS-based test, the data is generated from model (\ref{DGP}), with the stationary process $x_t$ specified as (\ref{x}), and by further setting
\begin{equation}
...
N\left( 0,1\right)$ and we assign $\theta=0.999$. 
...
\end{equation}%
\end{enumerate}
{\hskip 1.5em}We implement the 10\% SS-POS-based test and other tests which are intended to be robust against
...
{\hskip 1.5em}Monte Carlo simulation results are presented in Figures \ref{fig: Power
comparison using different tests NormalC1}-\ref{fig: Power comparison using different tests Normal BreakC1}. These results correspond to different DGPs
...
\begin{center}
\subfigure{\includegraphics[scale=0.45]{NormalTheta0.999Rho0.eps}} %
\subfigure{\includegraphics[scale=0.45]{NormalTheta0.999Rho0.1.eps}} \\[0pt]
\subfigure{\includegraphics[scale=0.45]{NormalTheta0.999Rho0.5.eps}} %
\subfigure{\includegraphics[scale=0.45]{NormalTheta0.999Rho0.9.eps}}\\[0pt]
\end{center}
...
\theta =0.9$ in (\protect\ref{x}).}
\begin{center}
\subfigure{\includegraphics[scale=0.45]{Student0.999Rho0.eps}} %
\subfigure{\includegraphics[scale=0.45]{Student0.999Rho0.1.eps}} \\[0pt]
\subfigure{\includegraphics[scale=0.45]{Student0.999Rho0.5.eps}} %
\subfigure{\includegraphics[scale=0.45]{Student0.999Rho0.9.eps}}\\[0pt]
\end{center}
...
\theta =0.9$ in (\protect\ref{x}).}
\begin{center}
\subfigure{\includegraphics[scale=0.45]{CauchyTheta0.999Rho0.eps}} %
\subfigure{\includegraphics[scale=0.45]{CauchyTheta0.999Rho0.1.eps}} \\[0pt]
\subfigure{\includegraphics[scale=0.45]{CauchyTheta0.999Rho0.5.eps}} %
\subfigure{\includegraphics[scale=0.45]{CauchyTheta0.999Rho0.9.eps}}\\[0pt]
\end{center}
...
\begin{center}
\subfigure{\includegraphics[scale=0.45]{MixTheta0.999Rho0.eps}} %
\subfigure{\includegraphics[scale=0.45]{MixTheta0.999Rho0.1.eps}} \\[0pt]
\subfigure{\includegraphics[scale=0.45]{MixTheta0.999Rho0.5.eps}} %
\subfigure{\includegraphics[scale=0.45]{MixTheta0.999Rho0.9.eps}}\\[0pt]
\end{center}
...
corresponds to the case where the error term $\varepsilon _{t}$ follows a
mixture of normal and Cauchy distributions. The results suggest that the $t$-test is invalid, as it over rejects under the null hypothesis of unpredictability for all values of $\rho$. Moreover, the $10\%$
SS-POS test is again more powerful than the CD (1995), $t$-test, and the WT-test. The
...
\begin{center}
\subfigure{\includegraphics[scale=0.45]{BIV0.999Rho0.eps}} %
\subfigure{\includegraphics[scale=0.45]{BIV0.999Rho0.1.eps}} \\[0pt]
\subfigure{\includegraphics[scale=0.45]{BIV0.999Rho0.5.eps}} %
\subfigure{\includegraphics[scale=0.45]{BIV0.999Rho0.9.eps}}\\[0pt]
\end{center}
...
BreakC1} compare the power function of the $10\%$ SS-POS test, CD (1995) test, WT-test,
and $t$-test \ in the case where $\varepsilon _{t}$ follows normal
distribution with a break in variance. Figure \ref{fig: Power comparison using different tests
Normal BreakC1} shows that in the presence of break in variance, the WT-test and
the $t$-test are undersized, whereas $10\%$ SS-POS test and CD (1995) test control size.
In addition, $10\%$ SS-POS test has more power that the other tests. The
CD (1995) test has the second best power, whereas the $t$-test and WT-test are plagued with poor power. The
power of these tests improve when the correlation coefficient is increased $%
\rho .$ 
\FloatBarrier
\section{Empirical application \label{Empirical
ApplicationC1}}
{\hskip 1.5em}In this Section, we consider an empirical application of the proposed 10\% SS-POS tests to illustrate its practical relevance. Valuation ratios are widely considered as predictors of stock returns and are generally known to be persistent. Therefore, they fit well within the framework of our study. In what follows, we specifically divert our attention to an application in the context of stock return predictability using the said ratios.

\subsection{Stock return predictability using valuation ratios  \label{StockReturnPredictability}}

{\hskip 1.5em}Many studies have investigated the predictive power of valuation ratios on excess stock returns. Dividend-price and earnings-price ratios
...
{\hskip 1.5em}The projection technique based on the proposed 10\% SS-POS test is used to build simultaneous confidence sets for the parameters of the regressions of the excess returns against the
...
{\hskip 1.5em}In this paper, we propose point-optimal sign-based tests for inference in linear
and nonlinear predictive regressions in the presence of stochastic (or fixed) regressors.
One motivation is to build valid (control the size whatever the
...
earning-price ratio, etc.) are known to be persistent with errors that are correlated with
the shock in the returns. This makes the classical predictability tests
invalid, particularly when the sample size is small or moderate. In
addition, the proposed sign-based tests are exact, distribution-free and
robust against heteroskedasticity of unknown form, and further allow for serial (nonlinear) dependence. Additionally, they may be inverted to build confidence regions for the parameters of the regression function. Since the
point-optimal sign-based tests depend on the alternative hypothesis, an adaptive
approach based on the split-sample technique is suggested to choose
...
intended to be robust against heteroskedasticity. We considered different
