...
Under model (\ref{model}) and assumption (\ref{eq: mediane}), the variables $s(\varepsilon_1),\cdots,s(\varepsilon_n)$ are i.i.d conditional on $X$, according to the distribution
\[
P[s(\varepsilon_1)=1\mid X]=P[s(\varepsilon_1)=0\mid X]=\frac{1}{2},\quad t=1,\cdots,n
\]
This result holds true iff  for any combination of $t=1,\cdots,n$ there is a permutation $\pi: i\rightarrow j$ such that the mediangale assumption holds for $j$. Then the signs $s(\varepsilon_1),\cdots,s(\varepsilon_n)$ are i.i.d.

\noindent\textbf{Proof:} See Appendix.
...
\begin{proposition}
\label{proposition1C1} Under assumptions (\ref{eq: mediane}) and (\ref{model}%
), let $H_{0}$ and $H_{1}$ be defined by (\ref{eq: hypothesis1}) - (\ref%
{alternative})$,$%
\begin{equation*}
SL_{n}(\beta _{1})=\sum\limits_{t=1}^{n}a_{t}(\beta _{1})s(y_{t}),
\end{equation*}%
where, for $t=1,...,n,$%
\begin{equation}
a_{t}(\beta _{1})=\ln \left\{ \frac{P\left[ y_{t}\geq 0\mid \text{%
\b{S}}_{t-1},X\right] }{P\left[ y_{t}<0\mid \text{\b{S}}_{t-1},X\right] 
}\right\} \,,  \label{weights1}
\end{equation}%
and suppose the constant $c_{1}(\beta _{1})$ satisfies $P\left[
\sum_{t=1}^{n}a_{t}(\beta _{1})s(y_{t})>c_{1}(\beta _{1})\right] =\alpha $
under $H_{0},$ with $0<\alpha <1.$ Then the test that rejects $H_{0}$ when 
\begin{equation}
SL_{n}(\beta _{1})>c_{1}(\beta _{1})  \label{eq: SLn critical region}
\end{equation}%
...
\end{proposition}
\begin{sloppypar}
Notice that the calculation of the test statistic $SL_{n}(\beta _{1})$
depends on the weights $a_{t}(\beta _{1}),$ which in turn depends on the
calculation of the conditional probabilities $P\left[ y_{t}\geq 0\mid \text{\b{S%
}}_{t-1},X\right] $ and $P\left[ y_{t}<0\mid \text{\b{S}}%
_{t-1},X\right] .$ The latter terms are not easy to compute and involves the
distribution of the joint process of signs $\big(s(y_{1}),...,s(y_{n})\big)'$ conditional on $X$, which is unknown. An alternative way to compute the
terms $P\left[ y_{t}\geq 0\mid \text{\b{S}}_{t-1},X\right] $ and $%
P\left[ y_{t}<0\mid \text{\b{S}}_{t-1},X\right] $ is to use
simulations, however again this will be time consuming as it requires to
simulate the joint distribution of the process of signs $\big(%
s(y_{1}),...,s(y_{n})\big)',$ which depends on the sample size $n$.
For all these reasons and to make the test statistic $SL_{n}(\beta _{1})$
feasible, we make the following assumption.
\end{sloppypar}
...
\begin{corollary}
\label{CorollaryC1} Under assumptions (\ref{eq: mediane}) and (\ref{model}),
let $H_{0}$ and $H_{1}$ be defined by (\ref{eq: hypothesis1}) - (\ref%
{alternative})$,$%
\begin{equation*}
...
_{1}'x_{0}\mid X\right] }{P\left[ \varepsilon_{1}<-\beta _{1}^{^{\prime
}}x_{0}\mid X\right] }\right\} ,\quad\tilde{b}_{1}(\beta _{1})=0
\end{equation*}%
and for $t=2,...,n,$%
\begin{equation*}
\begin{tabular}{l}
...
x_{t-2}\mid X\right] }{P\left[ \varepsilon_{t-1}<-\beta _{1}'
x_{t-2}\mid X\right] }}{\frac{P\left[ \varepsilon_{t}<-\beta _{1}'
x_{t-1},\text{ }\varepsilon_{t-1}<-\beta_1'x_{t-2}\mid X\right] }{P%
\left[ \varepsilon_{t-1}<-\beta_1'x_{t-2}\mid X\right] }}\right\} $ \\ 
\\ 
$\tilde{b}_{t}(\beta _{1})=\ln \left\{ \frac{1-\left( \frac{P\left[
\varepsilon_{t}<-\beta_1'x_{t-1}\mid X\right] }{1-P\left[
\varepsilon_{t-1}<-\beta_1'x_{t-2}\mid X\right] }-\frac{P\left[ 
\text{ }\varepsilon_{t-1}<-\beta_1'x_{t-2},\text{ }\varepsilon_{t}<-\beta
_{1}'x_{t-1}\mid X\right] }{1-P\left[ \varepsilon_{t-1}<-\beta
_{1}'x_{t-2}\mid X\right] }\right) }{\frac{P\left[
\varepsilon_{t}<-\beta_1'x_{t-1}\mid X\right] }{1-P\left[
\varepsilon_{t-1}<-\beta_1'x_{t-2}\mid X\right] }-\frac{P\left[ 
\text{ }\varepsilon_{t-1}<-\beta_1'x_{t-2},\text{ }\varepsilon_{t}<-\beta
_{1}'x_{t-1}\mid X\right] }{1-P\left[ \varepsilon_{t-1}<-\beta
_{1}'x_{t-2}\mid X\right] }}\right\} -\ln \left\{ \frac{1-\frac{%
...
\end{tabular}%
\end{equation*}%
and suppose the constant $\tilde{c}_{1}(\beta _{1})$ satisfies $P%
\left[ \sum\limits_{t=1}^{n}\tilde{a}_{t}(\beta _{1})\text{ }%
s(y_{t})+\sum\limits_{t=1}^{n}\tilde{b}_{t}(\beta _{1})\text{ }%
s(y_{t})s(y_{t-1})>\tilde{c}_{1}(\beta _{1})\right] =\alpha $ under $H_{0},$
with $0<\alpha <1.$ Then the test that rejects $H_{0}$ when 
\begin{equation}
\tilde{S}L_{n}(\beta _{1})>\tilde{c}_{1}(\beta _{1})
...
parameter vector $\beta $ be identified. We consider the problem of testing
the null hypothesis%
\begin{equation}
H(\beta _{0}):\beta =\beta _{0}  \label{h01}
\end{equation}%
against the alternative hypothesis%
\begin{equation}
H(\beta _{1}):\beta =\beta _{1}\text{.}  \label{h02}
\end{equation}%
A test for $H(\beta _{0})$ against $H(\beta _{1})$ can be constructed as in
...
transformed model%
\begin{equation*}
\tilde{y}_{t}=g(x_{t-1},\,\beta ,\,\beta _{0})+\varepsilon_{t},
\end{equation*}%
where $\tilde{y}_{t}=y_{t}-f(x_{t-1},\beta _{0})$ and\ $g(x_{t-1},\beta
,\beta _{0})=f(x_{t-1},\beta )-f(x_{t-1},\beta _{0})$. Thus, testing $%
H(\beta _{0})$ against $H(\beta _{1})$ is equivalent to testing%
\begin{equation*}
\bar{H}_{0}:g(x_{t-1},\,\beta ,\beta _{0})=0,\text{\ for }t=1,...,n,
\end{equation*}%
against%
\begin{equation*}
\bar{H}_{1}:g(x_{t-1},\beta ,\beta _{0})=f(x_{t},\beta _{1})-f(x_{t},\beta
_{0}),\;\text{for }t=1,...,n.
\end{equation*}%
For $\tilde{U}(n)=(s(\tilde{y}_{1}),...,s(\tilde{y}_{n}))',$
where, for $1\leq t\leq n,$%
\begin{equation*}
s(\tilde{y}_{t})=\left\{ 
\begin{tabular}{l}
...
_{t=1}^{n}$ conditional on $X$ is given by:%
\begin{equation*}
L\left( \tilde{U}(n),\beta \right) =P\left[ s(\tilde{y}%
_{1})=\tilde{s}_{1},...,s(\tilde{y}_{n})=\tilde{s}_{n}\mid X\right] =\prod\limits_{t=1}^{n} P\left[ s(\tilde{y}_{t})=\tilde{s}_{t}\mid 
\text{\~{S}}_{t-1},X\right] ,
\end{equation*}%
with 
\begin{equation*}
...
\end{equation*}%
where each $\tilde{s}_{t}$, for $1\leq t\leq n$, takes two possible values $%
0 $ and $1$. Thus, we can use the result of proposition \ref{proposition1C1}
to derive a sign-based test for the null hypothesis $H(\beta _{0})$ against
the alternative hypothesis $H(\beta _{1})$, which leads to the following proposition:

\begin{proposition}\label{proposition2C1}
Under assumptions (\ref{eq: mediane}) and (\ref{nonlinearC1}),let $H(\beta
_{0})$ and $H(\beta _{1})$ be defined by $(\ref{h01})$ - $(\ref{h02}),$%
\begin{equation*}
SN_{n}(\beta _{0}|\beta _{1})=\sum\limits_{t=1}^{n}a_{t}(\beta _{0}|\beta
_{1})s\left( y_{t}-f(x_{t-1},\beta _{0})\right) 
\end{equation*}%
where, for $t=1,...,n,$%
\begin{equation*}
a_{t}(\beta _{0}|\beta _{1})=\ln \left\{ \frac{P\left[ \tilde{y}%
_{t}\geq 0\mid \text{\~{S}}_{t-1},X\right] }{P\left[ \tilde{y}%
_{t}<0\mid \text{\~{S}}_{t-1},X\right] }\right\} \,,
\end{equation*}%
and suppose the constant $c_{1}(\beta _{0},\,\beta _{1})$ satisfies $%
P\left[ \sum\limits_{t=1}^{n}a_{t}(\beta _{0}|\beta _{1})\text{ }s\left(
y_{t}-f(x_{t},\beta _{0})\right) >c_{1}(\beta _{0},\,\beta _{1})\right]
=\alpha $ under $H(\beta _{0}),$ with $0<\alpha <1.$ Then the test that
rejects $H(\beta _{0})$ when 
\begin{equation*}
SN_{n}(\beta _{0}|\beta _{1})>c_{1}(\beta _{0},\,\beta _{1})
\end{equation*}%
...
SS-POS test is again more powerful than CD (1995), T-test, and the WT-test. The
...
&=&\ln \left\{ \frac{1-P\left[ \varepsilon_{1}<-\beta _{1}^{^{\prime
}}x_{0}\mid X\right] }{P\left[ \varepsilon_{1}<-\beta_1'x_{0}\mid X\right] 
}\right\}\\
\tilde{b}_{1}&=&\ln \left\{ \frac{P\left[ y_{1}\geq 0\mid X\right] }{%
P\left[ y_{1}<0\mid X\right] }\right\} -\ln \left\{ \frac{P%
\left[ y_{1}\geq 0\mid X\right] }{P\left[ y_{1}<0\mid X\right] }\right\} =0
