...
Under model (\ref{model}) and assumption (\ref{eq: mediane}), the variables $s(\varepsilon_1),\cdots,s(\varepsilon_T)$, and in turn $s(y_1),\cdots,s(y_T)$ under the null hypothesis of unpredictability, are i.i.d conditional on $X$ according to the distribution
\[
...
{\hskip 1.5em}In this paper, we proposed point-optimal sign-based tests for inference in linear
...
robust against heteroskedasticity of unknown form and allow for serial (nonlinear) dependence. Additionally, they may be inverted to build
