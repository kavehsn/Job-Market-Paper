...
simulations, however this will be computationally burdensome, as it requires tge
...
Thus, the under the null hypothesis, the distribution of the test statistic $\widetilde{SL}_{T}(\bm{\beta}_{1})$
...
\]
 ...
 A special case is where $\varepsilon_1,\varepsilon_2,\cdots,\varepsilon_{T-1},\varepsilon_T$ are distributed according to Student's $t$ distribution $t(\nu)$, with $\nu$ degrees of freedom. As suggested earlier, since the errors exhibit serial nonlinear dependence, we may calculate the bivariate probabilities using \textquotedblleft jointly-symmetric\textquotedblright{ }copulas by considering the Archimedean Frank, Clayton or Gumbel as the copula family [see \citet{joe2014dependence}]. Alternatively, we may evaluate the bivariate probabilities $P\left[\varepsilon_{t-1}<\cdot ,\-
...
\end{tabular}%
\end{equation*}%
where $\tau_{\nu}(.)$ is the Student's $t$ distribution function with $\nu$ degrees of freedom, $\bm{\mathcal{T}}_{\nu}(.,.)$ is the bivariate $t$ distribution with $\nu$ degrees of freedom, and with location and shape parameters $0$ and $I$ respectively, where $0$ is a zero vector and $I$ is the identity correlation matrix.
...
\begin{enumerate}
\item Compute the test statistic $\widetilde{SL}_{T}(\bm{\beta}_{1})$ based on the
observed data, say $\widetilde{SL}_{T}^{0}(\bm{\beta}_{1})$;

\item Generate a sample $\left\{ y_{t}\right\} _{t=1}^{T}$ of length $T$
under the null $H_{0}$ and compute $\widetilde{SL}_{T}^{j}(\bm{\beta}_{1})$ using
that generated sample;

\item Choose $B$ such that $\alpha (B+1)$ is an integer and repeat steps 1-2
$B$ times;

\item Compute the $(1-\alpha )\%$ quantile, say $\tilde{c}_{1}(\bm{\beta}_{1})$%
, of the sequence $\{\widetilde{SL}_{T}^{j}(\bm{\beta}_{1})\}_{j=1}^{B}$;

\item Reject the null hypothesis at level $\alpha $ if $\widetilde{SL}_{T}^{0}(\bm{\beta}
...
$ is calculated using the second part of the sample and the alternative $\bm{\beta}_1$ estimated using the first subsample. By adopting this technique, size control is easier and the power function of the POS-based test traces out the power envelope.
...
\end{equation*}
and where $y_{(2)}$ and $X_{(2)}$ which correspond to the second subsample are used to calculate the test statistic:
\begin{align*}
\widehat{SN}_{T}(\bm{\beta} _{0}\mid\bm{\beta}_{(1)})&=\sum\limits_{t=T_1+1}^{T}\tilde{a}%
_{t}(\bm{\beta}_{0}\mid\bm{\beta}_{(1)})s(y_{t}-f(\bm{x}_{t-1},\bm{\beta}
_{0}))\\
